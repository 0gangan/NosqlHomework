        ...
        <div v-else class="messages">
          ...
          >
            ...
            <div class="bubble-wrap">
              ...
              <!-- 图表展示区域 -->
              <div v-if="msg.role === 'assistant' && msg.charts && msg.charts.length" class="charts-area">
                <div
                ...
                >
                  ...
                  <div
                    :ref="el => { if (el) setChartRef(idx, ci, el) }"
                    class="chart-box"
                  />
                </div>
              </div>
...
async function sendMessage() {
  ...
  try {
    ...
    assistantMsg.charts = result.charts || []
...
// ===== ECharts 图表渲染 =====
const chartRefs = ref({})  // key: "msgIdx-chartIdx" -> DOM element
const chartInstances = {}   // key: "msgIdx-chartIdx" -> echarts instance

function setChartRef(msgIdx, chartIdx, el) {
  const key = msgIdx + '-' + chartIdx
  if (chartRefs.value[key] !== el) {
    chartRefs.value[key] = el
    // 延迟渲染，等待 DOM 挂载完成
    nextTick(() => {
      renderChart(msgIdx, chartIdx)
    })
  }
}
  ...
  if (chart.type === 'line') {
    return {
      ...
      xAxis: {
        type: 'category',
        data: chart.xAxis || [],
        axisLabel: { rotate: chart.xAxis && chart.xAxis.length > 6 ? 30 : 0 }
      },
      yAxis: { type: 'value' },
...
function renderChart(msgIdx, chartIdx) {
  const key = msgIdx + '-' + chartIdx
  const el = chartRefs.value[key]
  if (!el) return

  const msg = messages.value[msgIdx]
  if (!msg || !msg.charts || !msg.charts[chartIdx]) return
  ...
  const chart = msg.charts[chartIdx]
...
// 当 messages 变化时重新渲染所有图表的观测
watch(messages, () => {
  nextTick(() => {
    Object.keys(chartRefs.value).forEach(key => {
      const [msgIdx, chartIdx] = key.split('-').map(Number)
      renderChart(msgIdx, chartIdx)
    })
  })
}, { deep: true })
...
function renderMarkdown(text) {
  if (!text) return ''
  // 去除可能的 chart 代码块（后端已解析，此处作为安全兜底）
  let html = text.replace(/```chart[\s\S]*?```/g, '')
  // 先转义 HTML 原文 < >
  html = html
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
