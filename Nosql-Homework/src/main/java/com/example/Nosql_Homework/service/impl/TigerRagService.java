...
public class TigerRagService {
    ...
    public RagAnswer chat(String sessionId, String query, Integer reqTopK, Double reqMinScore) {
        ...
        if (answer != null && !answer.isEmpty()) {
            ParsedAnswer parsed = parseCharts(answer);
            answer = parsed.textAnswer;
    ...
    /** 图表数据 DTO —— 从 LLM 回答中解析出的 ```chart 块 */
    @lombok.Data
    ...
    /**
     * 从 LLM 回答中提取 ```chart ... ``` 代码块，解析为 ChartData 列表，
     * 并从原文中移除这些代码块。
     */
    private ParsedAnswer parseCharts(String rawAnswer) {
        ParsedAnswer result = new ParsedAnswer();
        result.charts = new ArrayList<>();

        if (rawAnswer == null || rawAnswer.isEmpty()) {
            result.textAnswer = rawAnswer;
            return result;
        }
        ...
        Matcher matcher = chartPattern.matcher(rawAnswer);

        StringBuffer cleanText = new StringBuffer();
        while (matcher.find()) {
            String jsonStr = matcher.group(1).trim();
            try {
                Map<String, Object> chartMap = objectMapper.readValue(
                        jsonStr,
                        new TypeReference<Map<String, Object>>() {}
                );
                ChartData chart = mapToChartData(chartMap);
                if (chart != null) {
                    result.charts.add(chart);
                }
            } catch (Exception e) {
                log.warn("[Tiger-RAG] 解析 chart JSON 失败 (将忽略此图表): {} - json={}",
                        e.getMessage(), truncate(jsonStr, 200));
            }
            // 移除代码块，替换为空字符串
        ...
        // 清理多余空行
        result.textAnswer = cleanText.toString().replaceAll("\\n{3,}", "\n\n").trim();
        return result;
    }

    /**
     * 将解析出的 Map 转换为 ChartData 对象，做基本校验
     */
    @SuppressWarnings("unchecked")
    private ChartData mapToChartData(Map<String, Object> map) {
        String type = (String) map.get("type");
        if (type == null || (!"line".equals(type) && !"pie".equals(type) && !"bar".equals(type))) {
            log.warn("[Tiger-RAG] 图表类型无效: {}", type);
            return null;
        }

        ChartData chart = new ChartData();
        chart.setType(type);
        chart.setTitle((String) map.getOrDefault("title", ""));

        // 解析 xAxis (line/bar 需要)
        Object xAxisObj = map.get("xAxis");
        if (xAxisObj instanceof List) {
            List<String> xAxis = new ArrayList<>();
            for (Object item : (List<?>) xAxisObj) {
                xAxis.add(item != null ? item.toString() : "");
            }
            chart.setXAxis(xAxis);
        }

        // 解析 series
        Object seriesObj = map.get("series");
        if (seriesObj instanceof List) {
            List<ChartSeries> seriesList = new ArrayList<>();
            for (Object item : (List<?>) seriesObj) {
                if (item instanceof Map) {
                    Map<String, Object> sMap = (Map<String, Object>) item;
                    ChartSeries cs = new ChartSeries();
                    cs.setName((String) sMap.getOrDefault("name", ""));
                    cs.setData(sMap.get("data"));
                    seriesList.add(cs);
                }
            }
            if (!seriesList.isEmpty()) {
                ...
                log.warn("[Tiger-RAG] 图表 series 为空");
                return null;
            }
        } else {
            log.warn("[Tiger-RAG] 图表缺少 series 字段");
            return null;
        }
